...
**Тег `v1.0.0` пока не проставлен**: публичный релиз заблокирован
открытым лицензионным вопросом — WellBoard импортирует пакет
`mihomo` (GPL-3.0) для парсинга share-ссылок, а целевая лицензия
проекта — MIT. Решение у владельца (см. `docs/DECISIONS-phase7.md`
D34, `docs/DECISIONS.md` RK5/C6). Функционально это релиз-кандидат.
...
> **Open license question (блокирует публичный релиз и тег v1.0.0):**
> проект импортирует `github.com/metacubex/mihomo` (GPL-3.0) в
> `internal/convert` (парсер share-ссылок). Совместимость GPL-3.0 с
> MIT-распространением всего проекта не решена; эскалация у владельца
> (OPE-2271, `docs/DECISIONS-phase7.md` D34). До решения публичный
> репозиторий не открывать.
